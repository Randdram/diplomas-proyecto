#!/usr/bin/env python3
"""
Genera diplomas Y los sube automáticamente a Supabase
"""
...
def generar_y_subir_diploma(cur, alumno_id: int):
    """Genera un diploma para un alumno y lo sube a Supabase"""
    ...
    # Guarda PDF localmente
    pdf_name = f"DIPLOMA_{alumno_id}_{folio}.pdf"
    salida_path = Path(SALIDA_PDFS) / pdf_name
    ...
    url_supabase = None
    if SUPABASE_URL and SUPABASE_PUBLIC_BASE:
        try:
            url_supabase = upload_pdf(str(salida_path), f"diplomas/{pdf_name}")
            print(f"✅ Subido a Supabase: {url_supabase}")
        except Exception as e:
            print(f"⚠️ Error subiendo a Supabase: {e}")
            print(f"   Usando URL local en su lugar")
            url_supabase = f"/pdfs/{pdf_name}"
    else:
        url_supabase = f"/pdfs/{pdf_name}"
    ...
    cur.execute("""
        INSERT INTO diploma 
        (alumno_id, folio, fecha_emision, hash_sha256, estado, pdf_path, pdf_url)
        VALUES (%s, %s, %s, %s, 'VALIDO', %s, %s)
    """, (alumno_id, folio, fecha_emision, sha, str(salida_path), url_supabase))
    
    print(f"✅ {alumno_nombre} ({curp}) -> {pdf_name}")
    return True
...
def main():
    ...
    try:
        ...
        print(f"🚀 Generando y subiendo {len(alumnos)} diploma(s)...\n")
        
        for aid in alumnos:
            if generar_y_subir_diploma(cur, aid):
                conn.commit()
        
        print(f"\n🎉 Listo. {len(alumnos)} diploma(s) generado(s) y subido(s)")
